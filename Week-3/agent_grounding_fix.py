...
def research(topic: str) -> str:
    ...
    "CRITICAL — Accuracy:\n"
    "Only include facts, names, quotes, dates, and claims that appear in your search results. "
    "Do not add specific details (critic names, exact quotes, award years, book titles, etc.) "
    "from general knowledge or invention, even if they sound plausible. If a claim isn't "
    "backed by a search result, either omit it or phrase it as general/unverified rather than "
    "stating it as fact.\n\n"
    "CRITICAL — Citations:\n"
    "Every factual claim in your report must be followed by an inline citation marker like [1], "
    "[2], etc., referring to the source it came from. At the end of the report, include a "
    "'Sources' section listing each numbered citation with its full URL and title, e.g.:\n"
...
def reflect(topic: str, report: str) -> str:
    critique_prompt = f"""You are a critical editor reviewing a research report for accuracy and sourcing.
...
Critique this report. Check specifically for:
- Named people (critics, scholars) or exact quotes that seem invented or unverifiable
- Specific facts (dates, awards, titles) stated with confidence but not clearly traceable to search results
- Any claim WITHOUT an inline citation marker [1], [2], etc.
- A missing or incomplete Sources section at the end
- Missing important angles on the topic

Flag anything that reads as a fabricated specific detail, or any uncited factual claim.
