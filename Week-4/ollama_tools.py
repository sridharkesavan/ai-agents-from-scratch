...
# --- Same tool function from Week 1 (Open-Meteo, no API key needed) ---
...
# --- Tool schema, Ollama's format is very close to Claude's ---
tools = [
    {
        "type": "function",
        "function": {
...
if __name__ == "__main__":
    conversation = []  # persists across the whole session now
    while True:
        q = input("\nAsk something (or 'quit'): ")
        if q.lower() == "quit":
            break
        conversation.append({"role": "user", "content": q})
        answer, conversation = run_agent(conversation)
        print(answer)